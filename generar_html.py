...
<div class="container" id="app">
    ...
    <div class="tab-content" id="tab-temporal">
        ...
        <div class="filtros-card">
            ...
            <div class="filtro-group"><span>Desde:</span><input type="date" id="fechaDesde"></div>
            <div class="filtro-group"><span>Hasta:</span><input type="date" id="fechaHasta"></div>
            <button class="btn-filtrar" onclick="aplicarFiltroFechas()">Aplicar</button>
            <button class="btn-reset" onclick="resetFiltroFechas()">Limpiar</button>
            <span class="filtro-info" id="filtroInfo">Mostrando todos</span>
        </div>
        <div class="charts-row">
...
(function() {{
    ...
    kpiRow.innerHTML = segKeys.map(s => `
        <div class="kpi-card" style="border-top-color:${{segColors[s]||'#999'}};cursor:pointer" onclick="filterBySegment('${{s}}')" title="Ver clientes de ${{s}}">
            <div class="number">${{segmentStats[s].clientes}}</div>
            <div class="label">${{s}}</div>
            <div style="font-size:10px;color:#888;margin-top:4px">${{segmentStats[s].contratos}} contratos - ${{fmtMoney(segmentStats[s].facturado)}}</div>
        </div>
    `).join('');
    ...
    safeChart('chartSegPie', {{
        type:'doughnut',
        data:{{
            labels:segKeys.map(s=>s+' ('+segmentStats[s].clientes+')'),
            datasets:[{{data:segKeys.map(s=>segmentStats[s].clientes), backgroundColor:segKeys.map(s=>segColors[s]||'#9ca3af'), borderColor:'#fff', borderWidth:3}}]
        }},
        options:{{responsive:true, maintainAspectRatio:false, plugins:{{legend:{{position:'right', labels:{{font:{{size:12}}}}}}, tooltip:{{callbacks:{{label:ctx=>ctx.label+' — '+((ctx.parsed/(segKeys.reduce((a,s)=>a+segmentStats[s].clientes,0)))*100).toFixed(1)+'%'}}}}}}}}
    }});
...
// Set date range
const allDates = clientes.filter(c=>c.first_date).map(c=>c.first_date).concat(clientes.filter(c=>c.last_date).map(c=>c.last_date));
const minDate = allDates.length ? allDates.reduce((a,b)=>a<b?a:b) : '2023-01-01';
const maxDate = allDates.length ? allDates.reduce((a,b)=>a>b?a:b) : '2026-12-31';
document.getElementById('fechaDesde').value = minDate;
document.getElementById('fechaHasta').value = maxDate;
...
function aplicarFiltroFechas() {{
    ...
    filteredVip = vipClients.filter(c => {{
        if (desde && c.first < desde) return false;
        if (hasta && c.last > hasta) return false;
        return true;
    }});
    if (!filteredVip.length) {{
        document.getElementById('filtroInfo').textContent = '⚠ Sin resultados';
        document.getElementById('tbodyTemporal').innerHTML = '<tr><td colspan="7" style="text-align:center;padding:20px;color:#999;">Sin clientes en este rango</td></tr>';
        if (timelineChart) timelineChart.destroy();
        return;
    }}
    document.getElementById('filtroInfo').textContent = 'Mostrando '+filteredVip.length+' de '+vipClients.length+' VIP';
    updateTemporalUI(filteredVip);
}}

function resetFiltroFechas() {{
    document.getElementById('fechaDesde').value = '';
    document.getElementById('fechaHasta').value = '';
    filteredVip = [...vipClients];
    document.getElementById('filtroInfo').textContent = 'Mostrando todos';
    updateTemporalUI(filteredVip);
}}
